...
import { useLocation } from "react-router-dom";
import { getActiveSession } from "views/auth/session";

export default function Header() {
  const location = useLocation();
  const sessionData = getActiveSession() || {};
  const displayUserId = String(sessionData.userId || "").trim();
  const displayCostCenter = String(sessionData.costCenter || "").trim();
  // const [showProfileDropdown, setShowProfileDropdown] = useState(false);
  ...
  const breadcrumbMap = {
    ...
    "/admin/newpiv": ["PIV", "Genearate PIV"],
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-1 justify-end">
        ...
        <div className="relative">
          <button className="flex items-center gap-1 px-3 py-2 rounded hover:bg-yellow-200 transition">
            <span>{displayUserId || "N/A"}</span>
          </button>
        </div>
        <div className="relative">
          <button className="flex items-center gap-1 px-3 py-2 rounded hover:bg-yellow-200 transition">
            <span>{displayCostCenter || "N/A"}</span>
          </button>
        </div>
      </div>
    </header>
